...
from utils.config import SAVE_DIR_CV_SAMPLES, SAVE_DIR_CV_VIEWS, SAVE_DIR_CV_PROCESSED
# TODO: Need to implement cross validation splits for model assesment and model selection
# NOW: it splits data into train/test not train/test/val
...
def split_data_fold(n_fold, dataset):
    ...
    if not os.path.exists(SAVE_DIR_CV_SAMPLES+str(n_fold)):
        os.makedirs(SAVE_DIR_CV_SAMPLES+str(n_fold))
    ...
    for i in range(len(folds)):
        ...
        with open(SAVE_DIR_CV_SAMPLES+str(n_fold)+'/'+'Folds_'+str(n_fold)+'_'+dataset+'_fold_'+str(i)+'_train', 'wb') as f:
            pickle.dump(train_folds, f)
        with open(SAVE_DIR_CV_SAMPLES+str(n_fold)+'/'+'Folds_'+str(n_fold)+'_'+dataset+'_fold_'+str(i)+'_test', 'wb') as f:
            pickle.dump(test_folds, f) 

# Splits and saves views of train and test sets
def split_views(n_fold, dataset):
    
    if not os.path.exists(SAVE_DIR_CV_VIEWS+str(n_fold)):
        os.makedirs(SAVE_DIR_CV_VIEWS+str(n_fold))

    rep  = SAVE_DIR_CV_SAMPLES +str(n_fold)+'/'
    dest = SAVE_DIR_CV_VIEWS +str(n_fold)+'/'
...
def transform(n_fold, dataset):
    ...
    dest = SAVE_DIR_CV_VIEWS+str(n_fold)+'/'   
    if not os.path.exists(SAVE_DIR_CV_PROCESSED+str(n_fold)):
        os.makedirs(SAVE_DIR_CV_PROCESSED+str(n_fold))
    
    for cv in range(n_fold):        
        with open(SAVE_DIR_CV_SAMPLES+str(n_fold)+'/'+'Folds_'+str(n_fold)+'_'+dataset+'_fold_'+str(cv)+'_train','rb') as f:
            G_list_train_i = pickle.load(f)
        ...
        for v in range(n_views):
            ...
            with open(SAVE_DIR_CV_PROCESSED+str(n_fold)+'/'+dataset+'_view_'+str(v) +'_folds_'+ str(n_fold) + '_fold_'+str(cv)+'_train_pg','wb') as f:
                pickle.dump(train_list_pg, f)
            with open(SAVE_DIR_CV_PROCESSED+str(n_fold)+'/'+dataset+'_view_'+str(v) +'_folds_'+ str(n_fold) + '_fold_'+str(cv)+'_test_pg','wb') as f:
...
def transform_Data(n_fold, dataset):   
    ...
    if not os.path.exists(SAVE_DIR_CV_PROCESSED+str(n_fold)+'/'+dataset+'_view_'+str(0)+'_folds_'+ str(n_fold) +'_fold_'+str(0)+'_test_pg'):
