...
from models.gunet import GNet
import utils.analysis as analysis
from utils.helpers import GraphSampler

from utils.config import SAVE_DIR_MODEL_DATA, SAVE_DIR_CV_VIEWS, SAVE_DIR_TWO_SHOT_VIEWS
...
def get_args(num_shots=2, cv_number=5):
    ...
    parser.add_argument('-num_epochs', type=int, default=25, help='epochs') #35
...
def evaluate(dataset, model, args, model_name):
    ...
    with open(SAVE_DIR_MODEL_DATA+"gunet/Labels_and_preds/"+model_name+".pickle", 'wb') as f:
...
def train(args, train_dataset, val_dataset, model, model_name):
    ...
    path = SAVE_DIR_MODEL_DATA+'gunet/weights/W_'+model_name+'.pickle'
    ...
    with open(SAVE_DIR_MODEL_DATA+"gunet/training_loss/Training_loss_"+model_name+".pickle", 'wb') as f:
      pickle.dump(los_p, f)
    
    torch.save(model, SAVE_DIR_MODEL_DATA+"gunet/models/gunet_"+model_name+".pt")
    return test_acc
...
def cv_benchmark(dataset, view, cv_number):
    ...
    for i in range(cv):
        print("CV : ",i)
        with open(SAVE_DIR_CV_VIEWS+str(cv)+'/'+dataset+'_view_'+str(view)+'_folds_'+ str(cv) +'_fold_'+str(i)+'_train','rb') as f:
            train_set = pickle.load(f)
        with open(SAVE_DIR_CV_VIEWS+str(cv)+'/'+dataset+'_view_'+str(view)+'_folds_'+ str(cv) +'_fold_'+str(i)+'_test','rb') as f:
