...
from utils.config import SAVE_DIR_TWO_SHOT_SAMPLES, SAVE_DIR_TWO_SHOT_VIEWS, SAVE_DIR_TWO_SHOT_PROCESSED
...
def few_shot_splits(dataset):
    ...
    random.seed(0)
    used_indexes = []
    if not os.path.exists(SAVE_DIR_TWO_SHOT_SAMPLES):
        os.makedirs(SAVE_DIR_TWO_SHOT_SAMPLES)
    ...
    for shot_n in range(100):  
        ...
        with open(SAVE_DIR_TWO_SHOT_SAMPLES+dataset+'_Two_Shot_'+str(shot_n)+'_train', 'wb') as f:
            pickle.dump(train_folds, f)
        with open(SAVE_DIR_TWO_SHOT_SAMPLES+dataset+'_Two_Shot_'+str(shot_n)+'_test', 'wb') as f:
            pickle.dump(test_folds, f)

    with open(SAVE_DIR_TWO_SHOT_SAMPLES+'indexes_'+dataset, 'wb') as f:
                pickle.dump(used_indexes, f)
    used_indexes = []
    
# Splits the views of selected training and test sets.    
def few_shot_split_views(dataset):
    if not os.path.exists(SAVE_DIR_TWO_SHOT_VIEWS):
        os.makedirs(SAVE_DIR_TWO_SHOT_VIEWS)
    
    for shot_n in range(100):
        with open(SAVE_DIR_TWO_SHOT_SAMPLES +dataset+'_Two_Shot_'+str(shot_n)+'_train','rb') as f:
            G_list_train_i = pickle.load(f)
        with open(SAVE_DIR_TWO_SHOT_SAMPLES +dataset+'_Two_Shot_'+str(shot_n)+'_test','rb') as f:
        ...
        for v in range(n_views):
            ...
            with open(SAVE_DIR_TWO_SHOT_VIEWS + dataset + '_view_'+str(v)+ '_shot_' + str(shot_n) +'_train','wb') as f:
                pickle.dump(G_list_train_i_view_v, f)
            with open(SAVE_DIR_TWO_SHOT_VIEWS + dataset + '_view_'+str(v)+ '_shot_' + str(shot_n) + '_test','wb') as f:
...
def few_shot_transformer(dataset):
    ...
    if not os.path.exists(SAVE_DIR_TWO_SHOT_PROCESSED):
        os.makedirs(SAVE_DIR_TWO_SHOT_PROCESSED)
    
    for shot_n in range(100):
        
        with open(SAVE_DIR_TWO_SHOT_SAMPLES+dataset+'_Two_Shot_'+str(0)+'_train','rb') as f:
            G_list_train_i = pickle.load(f)
        ...
        for v in range(n_views):
            train_list_pg = []
            test_list_pg = []
            with open(SAVE_DIR_TWO_SHOT_VIEWS + dataset + '_view_'+str(v)+ '_shot_' + str(shot_n) +'_train','rb') as f:
                list_train = pickle.load(f)
            with open(SAVE_DIR_TWO_SHOT_VIEWS + dataset + '_view_'+str(v)+ '_shot_' + str(shot_n) +'_test','rb') as f:
            ...
            with open(SAVE_DIR_TWO_SHOT_PROCESSED+dataset+'_view_'+str(v)+'_shot_'+str(shot_n)+'_train_pg','wb') as f:
                pickle.dump(train_list_pg, f)
            with open(SAVE_DIR_TWO_SHOT_PROCESSED+dataset+'_view_'+str(v)+'_shot_'+str(shot_n)+'_test_pg','wb') as f:
...
def transform_Data_FewShot(dataset):
    ...
    np.random.seed(0)
    random.seed(0)  
    if not os.path.exists(SAVE_DIR_TWO_SHOT_PROCESSED+dataset+'_view_'+str(0)+'_shot_'+str(0)+'_train_pg'):
