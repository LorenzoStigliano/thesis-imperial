...
def lsp_cross_validation_3(model_args, G_list, view, model_name, cv_number, n_students, run=0):
    start = time.time() 
    print("Run : ",run)
    print("--------------------------------------------------------------------------")
    ...
    for cv in range(cv_number):
        ...
        num_classes = 2 
        for i in range(n_students):
            if model_args["model_name"] == "gcn_student_lsp_ensamble_3_test":
...
def train(model_args, train_dataset, val_dataset, students, student_names, threshold_value, model_name, cv, view, cv_number, run=0):
    ...
    for epoch in range(model_args["num_epochs"]):
        ...
        for _, data in enumerate(train_dataset):
            ...
            loss_teacher_student = losses_1.mean() + losses_2.mean() + losses_3.mean()
