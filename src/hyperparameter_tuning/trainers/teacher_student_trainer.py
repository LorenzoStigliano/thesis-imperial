    ...
    for cv in range(cv_number):
        ...
        if model_args["evaluation_method"] =='model_selection':
            ...
            test_dataset = torch.utils.data.DataLoader(
                ...
                batch_size = 1,  
                shuffle = False) 
            return test(test_dataset, student_model, model_args, threshold_value, name)

        if model_args["evaluation_method"] =='model_assessment':
            #Here we join the train and validation dataset
            return train(model_args, train_dataset, val_dataset, student_model, threshold_value, name,  cv, view, cv_number, run)
