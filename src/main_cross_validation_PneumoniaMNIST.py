...
def train_main_model(dataset, model, view, cv_number, model_args, run=0):
    ...
    new_folder(model, model_args["evaluation_method"], dataset=model_args["dataset"], backbone=model_args["backbone"])
...
def parrallel_run(run):
    ...
    for dataset_i in datasets:
        if dataset_i == "gender_data":
            for view_i in views:
                models = [gcn_PneumoniaMNIST_args] # "gcn", "gcn_student" "gcn_3_args" args  gcn_student_args gat_args
                for model in models:
                    for cv in [3, 5, 10]:
                        train_main_model(dataset_i, model["model_name"], view_i, cv, model, run)
        else:
            models = [gcn_PneumoniaMNIST_args, gcn_student_PneumoniaMNIST_args] # "gcn", "gcn_student" "gcn_3_args" args  gcn_student_args gat_args
            for model in models:
                for cv in [3, 5, 10]:
