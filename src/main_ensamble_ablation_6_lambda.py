...
from models.model_config_hyperparameter_sensitivity_lambda import * 
...
def parrallel_run(run):
    ...
    for dataset_i in datasets_asdnc:
        if dataset_i == "gender_data": 
            for view_i in views:
                models = [gcn_student_lsp_ensamble_4_args_6] #gcn_student_ensamble_4_args, gcn_student_ensamble_5_args]
                for model in models:
                    for cv in [3, 5, 10]:
